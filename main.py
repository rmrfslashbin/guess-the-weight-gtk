...
class GuessTheWeight(Gtk.Window):
    def __init__(self):
        ...
        selectorBoxes = Gtk.Box(spacing=6, orientation=Gtk.Orientation.VERTICAL)
        ...
        barWeightBox = Gtk.Box(spacing=6, orientation=Gtk.Orientation.HORIZONTAL)
        ...
        for bar in weights.weights["bars"]:
            ...
            if not buttonGroup:
                buttonGroup = Gtk.RadioButton.new_with_label_from_widget(None, f"{str(bar)} lbs bar")
                buttonGroup.connect("toggled", self.on_bar_weight_selected, bar)
                ...
                btn = Gtk.RadioButton.new_with_label_from_widget(buttonGroup, f"{str(bar)} lbs bar")
        ...
        weight_scale = Gtk.Scale(
            orientation=Gtk.Orientation.HORIZONTAL, adjustment=targetWeightAdjustment)
    ...
    def on_click_calculate(self, widget):
        ...
        if solution:
            ...
            if solution['tries'] == 1:
                tries = "try"
            self.solutionLabel.set_text(f"Finished in {solution['tries']} {tries}")
            ...
            for plate in solution["solution"]:
                ...
                self.solutionStore.append([str(plate["weight"]), plate["count"]])
